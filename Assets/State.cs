...
public class State
{
    ...
    public bool PlayerSneakUpBehind()
    {
        Vector3 direction = -(player.position - npc.transform.position);
...
public class Patrol : State
{
    ...
    public override void Enter()
    {
        ...
        for (int i = 0; i < GameEnvironment.Singleton.GetCheckpoints.Count; i++)
        {
            GameObject thisWaypoint = GameEnvironment.Singleton.GetCheckpoints[i];
    ...
    public override void Update()
    {
        ...
        {
            if (currentIndex >= GameEnvironment.Singleton.GetCheckpoints.Count - 1)
            {
                currentIndex = 0;
            } else
            {
            ...
            agent.SetDestination(GameEnvironment.Singleton.GetCheckpoints[currentIndex].transform.position);
        }

        if (PlayerSneakUpBehind())
        {
...
public class Safe : State
{
    ...
    {
        name = STATE.SAFE;
        safeCubePos = GameObject.FindGameObjectWithTag("safe").transform.position;
    }

    public override void Enter()
    {      
        anim.SetTrigger("isRunning");
        agent.SetDestination(safeCubePos);
        base.Enter();
    }
    ...
    {              
        if (agent.hasPath)
        {
            if (Vector3.Distance(npc.transform.position, safeCubePos) < 2)
            {
                nextState = new Patrol(npc, agent, anim, player);
                stage = EVENT.EXIT;
            }
